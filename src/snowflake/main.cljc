(ns snowflake.main
  "Kotodama WASM entrypoint for the Snowflake clean-room actor (L4) — Clojure port.
  ...
  facts (`snowflake.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "snowflake")
(def tier "L4")
...
(def entity-specs
  [{:entity "Warehouse" :plural "warehouses" :id-prefix "snowflak_war"
    :fields [:name :warehouseType :warehouseSize :scalingPolicy :autoSuspend :autoResume :state] :required [:name :warehouseType]
    :coerce {:autoSuspend :int :autoResume :bool} :refs {}}
   {:entity "Database" :plural "databases" :id-prefix "snowflak_dat"
    :fields [:name :kind :owner :retentionTime :isDefault] :required [:name :kind]
    :coerce {:retentionTime :int :isDefault :bool} :refs {}}
   {:entity "Schema" :plural "schemas" :id-prefix "snowflak_sch"
    :fields [:name :kind :databaseName :owner :managedAccess] :required [:name :kind]
    :coerce {:managedAccess :bool} :refs {}}
   {:entity "Table" :plural "tables" :id-prefix "snowflak_tab"
    :fields [:name :kind :databaseName :schemaName :rows] :required [:name :kind]
    :coerce {:rows :int} :refs {}}
   {:entity "Task" :plural "tasks" :id-prefix "snowflak_tas"
    :fields [:name :warehouse :state :owner] :required [:name :warehouse]
    :coerce {} :refs {}}
   {:entity "Role" :plural "roles" :id-prefix "snowflak_rol"
    :fields [:name :owner :comment] :required [:name :owner]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"snowflake.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "snowflake-compat" :tier tier :entities entities} 200])
...
(defn start! [] :snowflake-compat/ready)
